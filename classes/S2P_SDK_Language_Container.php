...
class S2P_SDK_Language_Container extends S2P_SDK_Error
{
    ...
    private static $DEFINED_LANGUAGES = [];

    private static $LANGUAGE_INDEXES = [];
    ...
    public function define_language($lang, array $lang_params)
    {
        ...
        $lang = self::prepare_lang_index($lang);
        if (empty($lang)
         || empty($lang_params) || !is_array($lang_params)
        ...
        if (empty($lang_params['files']) || !is_array($lang_params['files'])) {
            $lang_params['files'] = [];
        }
    ...
    public function load_language($lang)
    {
        ...
        if (!($lang = self::valid_language($lang))
         || !($lang_details = self::get_defined_language($lang))
         || empty($lang_details['files']) || !is_array($lang_details['files'])) {
     ...
     */
    public function s2p_t($index, array $args = [])
    {
     ...
     * @return string
     */
    public function s2p_tl($index, $lang, array $args = [])
    {
        if (!is_string($index)) {
        ...
        && !self::language_loaded($lang)) {
            if (!$this->load_language($lang)) {
                if (($error_arr = $this->get_error()) && !empty($error_arr['error_simple_msg'])) {
    ...
    public static function convert_to_utf8($file, $params = false)
    {
        ...
        if (!in_array(strtolower($file_mime), ['utf8', 'utf-8'])) {
