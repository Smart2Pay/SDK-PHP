...
if (!defined('S2P_SDK_DIR_METHODS') || !defined('S2P_SDK_DIR_PATH')) {
...
{
    public const ERR_METHODS = 1, ERR_INSTANTIATE_METHOD = 2;

    private $_method;

    private $_func;

    private $_method_details = [];
    ...
    public function init($module_params = false)
    {
        ...
        if (!@is_dir($methods_dir)
         || !@file_exists(S2P_SDK_DIR_METHODS.'S2P_SDK_Method.php')) {
            die('SDK is not correctly configured. Please check bootstrap script.');
        }
    ...
    {
        if (empty($method_obj) || !is_object($method_obj)
         || !($method_obj instanceof S2P_SDK_Method)
         || !($func_details = $method_obj->valid_functionality($func))) {
            return '';
        }
...
$api_parameters['get_variables'] = array(<?php
        ...
        if (!empty($func_details['get_variables']) && is_array($func_details['get_variables'])) {
            echo "\n";
            foreach ($func_details['get_variables'] as $var_arr) {
                ...
                if (!($type_arr = S2P_SDK_Scope_Variable::valid_type($var_arr['type']))) {
                    $type_arr = ['title' => '(Unknown_var_type)'];
                }

                if ($var_arr['type'] == S2P_SDK_Scope_Variable::TYPE_STRING
                 || is_string($var_arr['default'])) {
                ...
                if (!empty($var_arr['mandatory']) || !empty($var_arr['display_name'])) {
                    $var_str .= ' //';
...
$api_parameters['method_params'] = array(<?php
                ...
                if (!empty($func_details['request_structure'])) {
                    ...
                    if (($method_input_arr = $structure_obj->extract_info_from_response_array(['foobar' => 1], $extraction_arr))) {
                        $mandatory_arr = [];
                        ...
                        echo "\n".$this->display_method_params($method_input_arr, $mandatory_arr, $hide_keys_arr, ['structure_obj' => $structure_obj]);
                    }
                }
        ?>);
...
<?php
    ...
    private function display_method_params($method_params, $mandatory_arr, $hide_keys_arr, $params = false)
    {
        ...
        if (empty($params['structure_obj'])
         || !is_object($params['structure_obj'])
         || !($params['structure_obj'] instanceof S2P_SDK_Scope_Structure)) {
        ...
        foreach ($method_params as $param_name => $param_value) {
            ...
            && array_key_exists($param_name, $hide_keys_arr)) {
                continue;
            }
            ...
            if (!is_array($param_value)) {
                ...
                if (empty($structure_obj)
                 || !($path_name = $structure_obj->path_to_display_name($current_path, ['check_external_names' => false]))) {
                    $path_name = '';
                }
                ...
                if (!empty($mandatory_str) || !empty($path_name)) {
                    $var_str .= '// '.$mandatory_str.((!empty($mandatory_str) && !empty($path_name)) ? ' - ' : '').$path_name;
                }
            } else {
                $var_str .= 'array( ';
                ...
                if (!($array_str = $this->display_method_params($param_value,
                    (array_key_exists($param_name, $mandatory_arr) ? $mandatory_arr[$param_name] : []),
                    (array_key_exists($param_name, $hide_keys_arr) ? $hide_keys_arr[$param_name] : []),
                    $new_params))
                ) {
                    continue;
                }
