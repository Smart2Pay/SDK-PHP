...
{
    public const ENV_TEST = S2P_SDK_REST_TEST, ENV_LIVE = S2P_SDK_REST_LIVE, ENV_CUSTOM = S2P_SDK_REST_CUSTOM;

    public const ENTRY_POINT_REST = 1, ENTRY_POINT_CARDS = 2;

    public const ERR_ENVIRONMENT = 100, ERR_METHOD = 101, ERR_METHOD_FUNC = 102, ERR_PREPARE_REQUEST = 103, ERR_URL = 104, ERR_HTTP_METHOD = 105,
        ERR_APIKEY = 106, ERR_CURL_CALL = 107, ERR_PARSE_RESPONSE = 108, ERR_VALIDATE_RESPONSE = 109, ERR_CALL_RESULT = 110, ERR_SITE_ID = 111,
        ERR_ENTRY_POINT = 112;

    public const TEST_CARDS_URL = 'https://securetest.smart2pay.com',
        LIVE_CARDS_URL = 'https://secure.smart2pay.com';

    public const TEST_BASE_URL = 'https://paytest.smart2pay.com',
        LIVE_BASE_URL = 'https://globalpay.smart2pay.com';

    public const TEST_RESOURCE_URL = 'https://paytest.smart2pay.com',
     ...
     * @return bool|int
     */
    public function entry_point(?int $ent = null)
    {
        if ($ent === null) {
            return $this->_entry_point;
        }

        if (!in_array($ent, [self::ENTRY_POINT_REST, self::ENTRY_POINT_CARDS], true)) {
    ...
    public function environment(?string $env = null)
    {
        ...
        if (!in_array($env, [self::ENV_TEST, self::ENV_LIVE, self::ENV_CUSTOM], true)) {
     ...
     */
    public function get_call_result() : array
    {
    ...
    public function do_finalize(array $params = []) : ?array
    {
    ...
    public function do_call($params = false)
    {
        ...
        if (empty($params['user_agent']) || !is_string($params['user_agent'])) {
            $params['user_agent'] = '';
        }
        ...
        if (($hook_result = self::trigger_hooks('rest_api_prepare_request_after', ['api_obj' => $this, 'request_data' => $request_data]))
        ...
        if (empty($request_data['http_method'])
         || !$this->_request->set_http_method($request_data['http_method'])) {
        ...
        $call_params['userpass'] = ['user' => $site_id, 'pass' => $api_key];

        self::trigger_hooks('rest_api_call_before', ['api_obj' => $this]);

        if (!($request_result = $this->_request->do_curl($call_params))) {
            $request_result = null;
        }

        if (($hook_result = self::trigger_hooks('rest_api_request_result', ['api_obj' => $this, 'request_result' => $request_result]))
        && is_array($hook_result)) {
        ...
        if (!in_array($request_result['http_code'], S2P_SDK_Rest_API_Codes::success_codes())) {
            ...
            if (($http_title = S2P_SDK_Rest_API_Codes::valid_code($request_result['http_code']))) {
                $code_str .= ' ('.$http_title.')';
            }

            // Set a generic error as maybe we will get more specific errors later when parsing response. Don't throw this error yet...
            $this->set_error(self::ERR_CURL_CALL, self::s2p_t('Request responded with error code %s', $code_str), '', ['prevent_throwing_errors' => true]);
        ...
        if (($hook_result = self::trigger_hooks('rest_rest_api_response_data', ['api_obj' => $this, 'response_data' => $response_data]))
        && is_array($hook_result)) {
    ...
    private function _validate_base_url() : bool
    {
