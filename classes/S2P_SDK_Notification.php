...
{
    public const ERR_UNKNOWN_TYPE = 1, ERR_BODY = 2, ERR_JSON = 3, ERR_RESPONSE_OK = 4, ERR_AUTHENTICATION = 5;

    public const TYPE_PAYMENT = 1, TYPE_PREAPPROVAL = 2, TYPE_REFUND = 3, TYPE_DISPUTE = 4;

    /** @var string */
    private $_notification_buffer = '';

    /** @var S2P_SDK_Scope_Structure */
    private $_notification_object;

    /** @var array */
    private $_notification_array = [];
    ...
    public function check_authentication()
    {
        ...
        if (empty($api_config_arr['api_key']) || empty($api_config_arr['site_id'])) {
        ...
        if ($_SERVER['PHP_AUTH_USER'] != $api_config_arr['site_id']
         || $_SERVER['PHP_AUTH_PW'] != $api_config_arr['api_key']) {
        ...
        if (($methods_arr = S2P_SDK_Method::get_all_methods())) {
            foreach ($methods_arr as $method_name => $method_arr) {
                ...
                if (!($instance = $method_arr['instance'])
                 || !($notification_check = $instance->check_notification($notification_arr))) {
                    continue;
                }
    ...
    {
        if (empty($type)
         || !($types_arr = self::get_types()) || empty($types_arr[$type])) {
            return false;
        }
    ...
    public static function logf()
    {
        ...
        if (empty($log_size)) {
            fwrite($fil, "        Date        |    Identifier   |      IP         |  Log\n");
            fwrite($fil, "--------------------+-----------------+-----------------+---------------------------------------------------\n");
        }
        ...
        if (defined('S2P_SDK_NOTIFICATION_IDENTIFIER') && constant('S2P_SDK_NOTIFICATION_IDENTIFIER')) {
            $notification_identifier = constant('S2P_SDK_NOTIFICATION_IDENTIFIER');
        }

        @fwrite($fil, date('d-m-Y H:i:s').' | '
