...
if (!($notification_obj = S2P_SDK\S2P_SDK_Module::get_instance('S2P_SDK_Notification', $notification_params))
 || $notification_obj->has_error()) {
    if ((S2P_SDK\S2P_SDK_Module::st_has_error() && $error_arr = S2P_SDK\S2P_SDK_Module::st_get_error())
     || (!empty($notification_obj) && $notification_obj->has_error() && ($error_arr = $notification_obj->get_error()))) {
...
switch ($notification_type) {
    case $notification_obj::TYPE_PAYMENT:
        if (!($result_arr = $notification_obj->get_array())
         || empty($result_arr['payment']) || !is_array($result_arr['payment'])) {
        ...
        if (empty($payment_arr['merchanttransactionid'])
         || empty($payment_arr['status']) || empty($payment_arr['status']['id'])) {
    ...
    case $notification_obj::TYPE_PREAPPROVAL:
        if (!($result_arr = $notification_obj->get_array())
         || empty($result_arr['preapproval']) || !is_array($result_arr['preapproval'])) {
        ...
        if (empty($preapproval_arr['merchantpreapprovalid'])
         || empty($preapproval_arr['status']) || empty($preapproval_arr['status']['id'])) {
    ...
    case $notification_obj::TYPE_REFUND:
        if (!($result_arr = $notification_obj->get_array())
         || empty($result_arr['refund']) || !is_array($result_arr['refund'])) {
        ...
        if (empty($refund_arr['merchanttransactionid'])
         || empty($refund_arr['status']) || empty($refund_arr['status']['id'])) {
            S2P_SDK\S2P_SDK_Notification::logf('MerchantTransactionID or Status not provided.');
            S2P_SDK\S2P_SDK_Notification::logf('Input buffer: '.$notification_obj->get_input_buffer(), false);
            exit;
        }
    ...
    case $notification_obj::TYPE_DISPUTE:
        if (!($result_arr = $notification_obj->get_array())
         || empty($result_arr['dispute']) || !is_array($result_arr['dispute'])) {
        ...
        if (empty($dispute_arr['id']) || empty($dispute_arr['paymentid'])
         || empty($dispute_arr['status']) || empty($dispute_arr['status']['id'])) {
