...
{
    public const FUNC_LIST_ALL = 'list_all', FUNC_METHOD_DETAILS = 'method_details', FUNC_LIST_COUNTRY = 'for_country',
        FUNC_ASSIGNED = 'assigned_methods', FUNC_ASSIGNED_COUNTRY = 'assigned_for_country';

    public const METHOD_KLARNA_INVOICE = 75, METHOD_KLARNA_PAYMENTS = 1078;
        ...
        switch ($call_result['response']['func']) {
            case self::FUNC_METHOD_DETAILS:
                if (!empty($call_result['response']['response_array']['method'])) {
                    $return_arr['custom_validators'] = [];
                    $return_arr['custom_validators']['payment'] = [];
                    $return_arr['custom_validators']['recurrent'] = [];
                    ...
                    $payment_request_arr = $variable_obj->nullify(null, ['check_external_names' => false, 'nullify_full_object' => true]);

                    $we_have_validators = false;
                    if (!empty($call_result['response']['response_array']['method']['validatorspayin'])
                    && is_array($call_result['response']['response_array']['method']['validatorspayin'])) {
                        $custom_validators = [];
                        foreach ($call_result['response']['response_array']['method']['validatorspayin'] as $validator_arr) {
                            if (($custom_validator = self::extract_method_validator($validator_arr, $payment_request_arr))) {
                                if (!($transform_result = $variable_obj->transform_keys(['Payment' => $custom_validator['sources']], null, ['check_external_names' => true]))
                                 || !is_array($transform_result)
                                 || empty($transform_result['payment']) || !is_array($transform_result['payment'])) {
                                    continue;
                                }
                    ...
                    && is_array($call_result['response']['response_array']['method']['validatorsrecurrent'])) {
                        $custom_validators = [];
    ...
    {
        if (empty($validator_arr) || !is_array($validator_arr)
         || empty($validator_arr['source'])) {
            return false;
        }

        if (empty($payment_request_arr) || !is_array($payment_request_arr)) {
            $pay_request_obj = new S2P_SDK_Structure_Payment_Request();
            $variable_obj = new S2P_SDK_Scope_Variable($pay_request_obj->get_definition());
        ...
        foreach ($sources_arr as $source) {
            ...
            while (!empty($parts_arr) && is_array($parts_arr) && ($key = array_shift($parts_arr))) {
                if (!is_array($node_arr)
                 || !array_key_exists($key, $node_arr)) {
                    $path_found = false;
                    break;
                }
            ...
            if (is_array($return_node_arr) && empty($return_node_arr)) {
                $return_node_arr = '';
            }
