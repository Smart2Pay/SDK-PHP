...
if (!($submit_result = $demo->handle_submit($submit_params))) {
    ...
    if ($demo->has_error() && ($error_arr = $demo->get_error())) {
        $submit_result['errors_arr'] = [$error_arr['display_error']];
    } else {
        $submit_result['errors_arr'] = [$demo::s2p_t('Error handling form submit.')];
    }
}
...
if (($form_result = $demo->get_form($form_params))) {
    if (!empty($form_result['form_messages'])) {
        ...
        if (!empty($form_result['form_messages']['errors_arr']) && is_array($form_result['form_messages']['errors_arr'])) {
        ...
        if (!empty($form_result['form_messages']['success_arr']) && is_array($form_result['form_messages']['success_arr'])) {
        ...
        if (!empty($form_result['form_messages']['warnings_arr']) && is_array($form_result['form_messages']['warnings_arr'])) {
